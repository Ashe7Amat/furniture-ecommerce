...
if (esReal) {
  throw new Error(
    'STRIPE_SECRET_KEY es una clave REAL: este test solo puede ejecutarse con una clave de prueba (sk_test_...).'
  );
}
...
const carrito = (piezas) =>
  Array.from({ length: piezas }, (_, i) => ({
    productId: uuid(i),
    modalidad: i % 2 ? 'alquiler' : 'compra'
  }));
const comprador = {
  nombre: 'Ana Martínez',
  email: 'ana@ejemplo.com',
  telefono: '600123456',
  direccion: 'Calle Mayor 15, 08001 Barcelona',
  notas: 'n'.repeat(500)
};

describe(
  'Stripe (modo test): límites de la metadata',
  {
    skip: esDePrueba ? false : 'No hay una STRIPE_SECRET_KEY de prueba (sk_test_...) en server/.env'
  },
  () => {
    let stripe;
    const creadas = [];
        ...
        line_items: [
          {
            price_data: {
              currency: 'eur',
              product_data: { name: 'Prueba de contrato Nave 5 (no cobrar)' },
              unit_amount: 100
            },
            quantity: 1
          }
    ...
    test('un valor de 500 caracteres se acepta y uno de 501 se rechaza (MAX_VALOR)', async () => {
      ...
      await assert.rejects(
        crearSesion({ valor: 'x'.repeat(501) }),
        (error) => rechazadaPorStripe(error) && /500/.test(error.message)
      );
    });
    ...
    test('100 piezas y notas de 500 caracteres se aceptan, y Stripe devuelve la metadata exactamente como se envió', async () => {
      ...
      assert.deepEqual(
        recuperada.metadata,
        enviada,
        'Stripe no debe alterar, recortar ni perder ninguna clave'
      );
      assert.deepEqual(leerItemsDeMetadata(recuperada.metadata), items);
    });

    test('un dato del comprador vacío no rompe la creación (se guarda vacío o se omite)', async () => {
      const enviada = construirMetadataPago({
        items: carrito(1),
        clienteInfo: { ...comprador, notas: '' }
      });
      ...
      assert.ok(
        recuperada.metadata.clienteNotas === '' || recuperada.metadata.clienteNotas === undefined
      );
...
if (!esDePrueba) {
  test(
    'sin STRIPE_SECRET_KEY de prueba el contrato con Stripe no se ha comprobado',
    { skip: 'ver la cabecera de este archivo' },
    () => {}
  );
}
